...
void alFxNew(ALFx *r, ALSynConfig *c, ALHeap *hp)
{
    ...
    r->delay = (ALDelay *) alHeapAlloc(hp, r->section_count, sizeof(ALDelay));
    r->base = (s16 *) alHeapAlloc(hp, r->length, sizeof(s16));
    r->input = r->base;
...
#define LENGTH	(d->output - d->input)
	    ...
	    d->rsval	 = 1.0;
	    d->rsdelta	 = 0.0;
	    d->rs 	 = (ALResampler *) alHeapAlloc(hp, 1, sizeof(ALResampler));
	    d->rs->state = (RESAMPLE_STATE *) alHeapAlloc(hp, 1, sizeof(RESAMPLE_STATE));
	    d->rs->delta = 0.0;
	    d->rs->first = 1;
	} else {
	    d->rs = 0;
	...
	if (param[j]) {
	    d->lp = (ALLowPass *) alHeapAlloc(hp, 1, sizeof(ALLowPass));
	    d->lp->fstate = (POLEF_STATE *) alHeapAlloc(hp, 1, sizeof(POLEF_STATE));
	    d->lp->fc = param[j++];
	    init_lpfilter(d->lp);
	} else {
	    d->lp = 0;
...
void alEnvmixerNew(ALEnvMixer *e, ALHeap *hp)
{
    ...
    e->state = (ENVMIX_STATE *) alHeapAlloc(hp, 1, sizeof(ENVMIX_STATE));
...
void alLoadNew(ALLoadFilter *f, ALDMANew dmaNew, ALHeap *hp) 
{
    ...
    f->state = (ADPCM_STATE *) alHeapAlloc(hp, 1, sizeof(ADPCM_STATE));
    f->lstate = (ADPCM_STATE *) alHeapAlloc(hp, 1, sizeof(ADPCM_STATE));
...
void alResampleNew(ALResampler *r, ALHeap *hp)
{
     ...
     */
    r->state = (RESAMPLE_STATE *) alHeapAlloc(hp, 1, sizeof(RESAMPLE_STATE));
    r->delta  = 0.0;
    r->first  = 1;
    r->motion = AL_STOPPED;
    r->ratio = 1.0;
