                    ...
                    s32 sampleOffset, Acmd *p) 
{
   ...
   aEnvMixer(ptr++, A_INIT | A_AUX, (u32) e->state); // Tracker64: aEnvMixer (ptr++, A_INIT | A_AUX, osVirtualToPhysical(e->state));
    }
    else
	aEnvMixer(ptr++, A_CONTINUE | A_AUX, (u32) e->state); // Tracker64: aEnvMixer(ptr++, A_CONTINUE | A_AUX, osVirtualToPhysical(e->state));
