...
void alSynNew(ALSynth *drvr, ALSynConfig *c)
{
    ...
    save = (ALSave *) alHeapAlloc(hp, 1, sizeof(ALSave));
    alSaveNew(save);
     ...
     */
    drvr->auxBus = (ALAuxBus_s *) alHeapAlloc(hp, 1, sizeof(ALAuxBus));
    drvr->maxAuxBusses = 1;
    sources = (ALFilter *) alHeapAlloc(hp, c->maxPVoices, sizeof(ALFilter *));
    alAuxBusNew(drvr->auxBus, sources, c->maxPVoices);

    /*
     * allocate and initialize the main bus.
     */
    drvr->mainBus = (ALMainBus_s *) alHeapAlloc(hp, 1, sizeof(ALMainBus));
    sources = (ALFilter *) alHeapAlloc(hp, c->maxPVoices, sizeof(ALFilter *));
    ...
    pvoices = (PVoice *) alHeapAlloc(hp, c->maxPVoices, sizeof(PVoice));
     ...
     */
    params = (ALParam *) alHeapAlloc(hp, c->maxUpdates, sizeof(ALParam));
    drvr->paramList = 0;
