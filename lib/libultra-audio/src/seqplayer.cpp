...
void alSeqpNew(ALSeqPlayer *seqp, ALSeqpConfig *c)
{
    ...
    seqp->curTime       = 0;
    seqp->initOsc       = (ALOscInit) c->initOsc;
    seqp->updateOsc     = (ALOscUpdate) c->updateOsc;
    seqp->stopOsc       = (ALOscStop) c->stopOsc;
    seqp->loopStart = 0;
    seqp->loopEnd   = 0;
    ...
    seqp->chanState = (ALChanState*) alHeapAlloc(hp, c->maxChannels, sizeof(ALChanState) );
    __initChanState(seqp);	/* sct 11/6/95 */
    
    /*
     * init the voice state array
     */
    voices = (ALVoiceState*) alHeapAlloc(hp, c->maxVoices, sizeof(ALVoiceState));
    seqp->vFreeList = 0;
     ...
     */
    items = (ALEventListItem*) alHeapAlloc(hp, c->maxEvents, sizeof(ALEventListItem));
...
ALMicroTime __seqpVoiceHandler(void *node)
{
	...
	switch (seqp->nextEvent.type) {
	  ...
	  case (AL_SEQP_SEQ_EVT):
	      ...
	      seqp->target = (ALSeq*) seqp->nextEvent.msg.spseq.seq;
